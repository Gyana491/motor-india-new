...
const getPostsByTag = async (tagSlug) => {
  ...
  // Then fetch posts with that tag ID for hindi post type
  const response = await fetch(
    `${process.env.BACKEND}/wp-json/wp/v2/hindi?tags=${tags[0].id}&_embed=true&per_page=100`,
    { next: { revalidate: 3600 } }
  )
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    alternates: {
      canonical: `${process.env.NEXT_PUBLIC_FRONTEND}/hindi/tag/${params.slug}`,
    },
    openGraph: {
      title: `${tag.name} - टैग्ड लेख | मोटर इंडिया`,
      description: tag.description || `मोटर इंडिया पर ${tag.name} से टैग किए गए सभी लेख`,
      url: `${process.env.NEXT_PUBLIC_FRONTEND}/hindi/tag/${params.slug}`,
...
export default async function TagPage({ params }) {
  ...
  return (
    <main className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <Link href="/hindi" className="text-red-600 hover:underline mb-6 inline-block">
          ← सभी हिंदी लेखों पर वापस जाएं
        </Link>
                ...
                <div className="flex items-center text-sm text-gray-500">
                  ...
                  {post.categories?.[0] && (
                    <>
                      ...
                      <Link 
                        href={`/hindi/category/${post.categories[0].slug}`}
