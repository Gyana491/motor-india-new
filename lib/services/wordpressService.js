...
class WordPressService {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 400 || response.status === 404) {
          console.warn(`Posts endpoint returned ${response.status}. No posts found.`);
          return [];
        }
  ...
  async getAllHindiPosts(page = 1, perPage = 100) {
    try {
      ...
      if (!response.ok) {
        if (response.status === 400 || response.status === 404) {
          console.warn(`Hindi posts endpoint returned ${response.status}. No Hindi posts found.`);
          return [];
        }
        console.warn(`Failed to fetch hindi posts: ${response.status}`);
        return [];
      }
  ...
  async getAllCategories() {
    try {
      ...
      if (!response.ok) {
        console.warn(`Categories endpoint returned ${response.status}. No categories found.`);
        return [];
      }
  ...
  async getAllHindiCategories() {
    try {
      ...
      if (!response.ok) {
        console.warn(`Hindi categories endpoint returned ${response.status}. This is expected if Hindi categories are not set up yet.`);
        return [];
      }
      ...
      console.warn('Hindi categories not available:', error.message);
      return [];
    }
  }
  ...
  async getAllTags() {
    try {
      ...
      if (!response.ok) {
        console.warn(`Tags endpoint returned ${response.status}. No tags found.`);
        return [];
      }
  ...
  async getAllHindiTags() {
    try {
      ...
      if (!response.ok) {
        console.warn(`Hindi tags endpoint returned ${response.status}. This is expected if Hindi tags are not set up yet.`);
        return [];
      }
      ...
      console.warn('Hindi tags not available:', error.message);
      return [];
    }
  }
