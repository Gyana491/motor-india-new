...
export async function generateMetadata({ params }) {
  ...
  if (!post) {
    return {
      title: 'पोस्ट नहीं मिला | मोटर इंडिया',
      description: 'अनुरोधित लेख नहीं मिल सका।'
    }
  }

  const metadata = {
    title: `${post.title} | मोटर इंडिया`,
    description: post.content.replace(/<[^>]*>/g, '').slice(0, 160),
    keywords: [...post.categories.map(cat => cat.name), ...post.tags.map(tag => tag.name), 'मोटर इंडिया', 'ऑटोमोटिव न्यूज़'].join(', '),
    robots: {
      index: true,
      follow: true,
      'max-snippet': -1,
    ...
    alternates: {
      canonical: `${process.env.NEXT_PUBLIC_FRONTEND}/hindi/${post.slug}`,
    },
    openGraph: {
      title: post.title,
      description: post.content.replace(/<[^>]*>/g, '').slice(0, 160),
      url: `${process.env.NEXT_PUBLIC_FRONTEND}/hindi/${post.slug}`,
      type: 'article',
    ...
    languages: {
      'hi': `${process.env.NEXT_PUBLIC_FRONTEND}/hindi/${post.slug}`,
      'en': `${process.env.NEXT_PUBLIC_FRONTEND}/articles`
    }
  }
...
const getPost = async (slug) => {
  const response = await fetch(`${process.env.BACKEND}/wp-json/wp/v2/hindi?slug=${slug}&_embed=true`, {
...
const getRecentPosts = async (excludeSlug) => {
  const response = await fetch(`${process.env.BACKEND}/wp-json/wp/v2/hindi?_embed=true&per_page=5`, {
...
export default async function SinglePost({ params }) {
  ...
  if (!post) {
    return (
      <main className="container mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold mb-6">पोस्ट नहीं मिला</h1>
        <p>क्षमा करें, आपका अनुरोधित लेख नहीं मिल सका।</p>
        <Link href="/hindi" className="text-red-600 hover:underline mt-4 inline-block">
  ...
  const structuredData = {
    ...
    mainEntityOfPage: {
      '@type': 'WebPage',
      '@id': `${process.env.NEXT_PUBLIC_FRONTEND}/hindi/${post.slug}`
    },
    inLanguage: 'hi'
  }
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <main className="lg:w-2/3">
            {/* Back button */}          <Link href="/hindi" className="text-red-600 hover:underline mb-6 inline-block">
              ...
              {post.categories.length > 0 && (
                <div className="ml-4 flex items-center">
                  ...
                  {post.categories.map((cat, index) => (
                    <span key={cat.id}>
                      {index > 0 && ", "}                    
                      <Link href={`/hindi/category/${cat.slug}`} className="text-red-600 hover:underline">
                        {cat.name}
                      </Link>
                    </span>
                  ))}
            ...
            <div className="mt-8 pt-6 border-t border-gray-200">
              ...
              <div className="flex space-x-4">
                <a 
                  href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(`https://motorindia.in/hindi/${post.slug}`)}`} 
                  target="_blank" 
                ...
                <a 
                  href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(`Check out: ${post.title}`)}&url=${encodeURIComponent(`https://motorindia.in/hindi/${post.slug}`)}`} 
                  target="_blank" 
                  rel="noopener noreferrer"
                ...
                <a 
                  href={`https://api.whatsapp.com/send?text=${encodeURIComponent(`${post.title}: https://motorindia.in/hindi/${post.slug}`)}`} 
                  target="_blank" 
                  rel="noopener noreferrer"
                ...
                <a 
                  href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`https://motorindia.in/hindi/${post.slug}`)}`} 
                  target="_blank" 
                  rel="noopener noreferrer"
            ...
            <div className="bg-gray-50 rounded-lg p-6 sticky top-8">
              ...
              {recentPosts.length > 0 ? (
                <div className="space-y-4">
                  {recentPosts.map(recentPost => (                  <Link 
                      key={recentPost.id} 
                      href={`/hindi/${recentPost.slug}`}
                ...
                <Link href="/hindi" className="text-red-600 hover:underline text-sm mt-4 inline-block">
                View all Hindi posts →
              </Link>
            </div>
          </aside>
        </div>
